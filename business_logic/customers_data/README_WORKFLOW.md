...
- `analyse_metier.ipynb`: exploratory analysis on sanitized data.
...
7. Prefer derived columns over destructive mutation of source columns.
8. Do not use `pandas` unless explicitly requested.

## VS Code setup checklist
1. Install/enable Gemini Code Assist extension.
2. Open folder `business_logic/customers_data` as workspace root.
...
5. Install runtime packages:
   ...
   - `xlsxwriter`
   - `altair`
6. In chat, attach only `analyse_metier.ipynb`, `main.py`, `../libs/*.py`, and `input/restaurant_orders_anonymous.csv`.
